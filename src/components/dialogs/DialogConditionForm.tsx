...
// Renders one dialog condition (a slide's dialogPlayConditions, or an answer's requests).
// Deliberately a separate component from ConditionForm/CONDITION_REGISTRY: dialog
// conditions are a smaller, different vocabulary — see src/schema/dialog-conditions.ts for
// why reusing the script condition form here produced wrong-shaped JSON for three names.

import type { DialogCondition } from '@/types/dialog'
import { DIALOG_CONDITION_REGISTRY, DIALOG_CONDITION_LIST } from '@/schema/dialog-conditions'
...
interface Props {
  ...
  onChange: (condition: DialogCondition) => void
  onRemove: () => void
}

export default function DialogConditionForm({ condition, onChange, onRemove }: Props) {
  const def = DIALOG_CONDITION_REGISTRY[condition.c]
  const isCustom = !def
  ...
  const updateType = (type: string) => {
    if (type === '__custom__') {
      ...
      const newDef = DIALOG_CONDITION_REGISTRY[type]
      onChange({ ...condition, c: type, p: newDef ? newDef.params.map(() => '') : [] })
    }
  }
    ...
    <div className="rounded-md border border-border bg-card p-3 space-y-3">
      ...
      <div className="flex items-start gap-2">
        <div className="flex-1 space-y-1">
          <Label className="text-xs">Dialog condition type</Label>
      ...
      {/* reqStatus inversion — the game only ever writes the literal string "false" here;
          an absent field is the normal (non-inverted) case. */}
      <label className="flex items-center gap-2 cursor-pointer">
        <Checkbox
          checked={condition.reqStatus === 'false'}
          onCheckedChange={(checked) =>
            onChange({ ...condition, reqStatus: checked === true ? 'false' : undefined })
          }
        />
        <span className="text-xs">
          Invert — slide/answer is skipped when this condition IS met
        </span>
      </label>
    </div>
  )
